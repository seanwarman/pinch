...
//
var samplesToInject = [ 'samples/*.wav' ];
...
module.exports.samplesToInject = samplesToInject.map((smpPath)=>{
  // If we're ignoring the file, make sure the ! is at the beginning of the path
  if (smpPath[0] === '!') {
    return require('path').join('!' + tmpPath, smpPath.substr(1));
  }
  return require('path').join(tmpPath, smpPath);
});
